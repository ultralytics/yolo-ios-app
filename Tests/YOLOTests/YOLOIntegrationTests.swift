...
class YOLOIntegrationTests: XCTestCase {
  ...
  func testCoordinateTransformations() {
    ...
    for normalizedBox in normalizedBoxes {
      ...
      XCTAssertEqual(
        pixelBox.size.width, normalizedBox.size.width * imageSize.width, accuracy: 0.001)
      XCTAssertEqual(
        pixelBox.size.height, normalizedBox.size.height * imageSize.height, accuracy: 0.001)
