...
class ViewController: UIViewController, YOLOViewDelegate {
  ...
  private func reorderYOLOModels(_ fileNames: [String]) -> [String] {
    ...
    // Put official models first so yolo11n comes first
    ...
    if entry.isLocalBundle {
      DispatchQueue.global().async { [weak self] in
        ...
        DispatchQueue.main.async {
          ...
          print("Calling yoloView.setModel with path: \(modelURL.path)")
      ...
      print("🔍 Checking remote model: \(key)")
      print("  - Display name: \(entry.displayName)")
      print("  - Remote URL: \(entry.remoteURL?.absoluteString ?? "nil")")
      print("  - Is downloaded: \(ModelCacheManager.shared.isModelDownloaded(key: key))")

      if ModelCacheManager.shared.isModelDownloaded(key: key) {
        print("📱 Loading cached model: \(key)")
  ...
  private func loadCachedModelAndSetToYOLOView(key: String, yoloTask: YOLOTask, displayName: String)
  {
    ...
    print("📁 Loading cached model from: \(localModelURL.path)")
    print("  - File exists: \(FileManager.default.fileExists(atPath: localModelURL.path))")
    ...
    DispatchQueue.main.async {
      ...
      self.yoloView.setModel(modelPathOrName: localModelURL.path, task: yoloTask) { result in
        switch result {
        case .success():
          print("✅ Successfully loaded cached model: \(displayName)")
          self.finishLoadingModel(success: true, modelName: displayName)
        case .failure(let error):
          print("❌ Failed to load cached model: \(displayName)")
          print("   Error: \(error)")
...
// Notification names for camera sharing
