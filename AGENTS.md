...
- `.mlpackage` models are never committed (gitignored); tests and the app get them from the `models-v1.0.0` release assets via `scripts/download-models.sh` (an Xcode "Download YOLO Models" build phase runs it locally and is skipped on GitHub Actions, where CI runs the script as its own step). Every official Core ML model uses a fixed 224 × 224 classification input or 640 × 640 input for every other task.
