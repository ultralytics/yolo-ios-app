    ...
    override func processObservations(for request: VNRequest, error: Error?) {
        if let results = request.results as? [VNCoreMLFeatureValueObservation] {
            ...
            if let prediction = results.first?.featureValue.multiArrayValue {
                ...
                let result = YOLOResult(orig_shape: inputSize, boxes: boxes, masks: nil, probs: nil, keypointsList:keypointsList, annotatedImage: nil, speed: 0, fps: 0, originalImage: nil, names: labels)
        ...
        do {
            ...
            if let results = request.results as? [VNCoreMLFeatureValueObservation] {
                ...
                if let prediction = results.first?.featureValue.multiArrayValue {
                    ...
                    let annotatedImage = drawPoseOnCIImage(ciImage: image, keypointsList: keypointsForImage, confsList: confsList, boundingBoxes: boxes, originalImageSize: inputSize)
                    updateTime()
                    return YOLOResult(orig_shape: inputSize, boxes: boxes, masks: nil, probs: nil, keypointsList:keypointsList, annotatedImage: annotatedImage, speed: self.t2, fps: 1 / self.t4, originalImage: nil, names: labels)
                }
            }
        } catch {
            print(error)
    ...
    -> [(box: Box, keypoints: Keypoints)]
    {
        ...
        DispatchQueue.concurrentPerform(iterations: numAnchors) { j in
            ...
            if confidence > confidenceThreshold {
                ...
                let boundingBox = CGRect(x: boxX, y: boxY,
        ...
        let results: [(Box, Keypoints)] = zip(boxScorePairs, filteredFeatures).map { (pair, boxFeatures) in
            ...
            let boxResult = Box(index: 0, cls: "person", conf: score, xywh: imageSizeBox, xywhn: normalizedBox)
