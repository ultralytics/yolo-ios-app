...
- ✅ **Simple API**: Easily utilize CoreML YOLO models with Python-like code syntax in [Swift](https://developer.apple.com/swift/).
...
Initialize the `YOLO` class with a valid Ultralytics YOLO model exported to CoreML format. You can load an official model from a remote URL, point to your own local `.mlpackage` or `.mlmodelc`, or reference a model already included in your app [bundle](https://developer.apple.com/documentation/foundation/bundle).
...
// Or load your own fine-tuned CoreML export from disk.
...
You can export models to the CoreML format yourself using the `ultralytics` Python package. This gives you more control over the export process, such as choosing different model sizes or export settings.
...
This script assumes you have the base [PyTorch](https://pytorch.org/) (`.pt`) models available. For detailed export options, refer to the [Ultralytics CoreML export documentation](https://docs.ultralytics.com/integrations/coreml/).

**Important Note on NMS:** The SDK supports both architectures. YOLO26 is NMS-free — export with `nms=False` (or omit the argument) and the Swift package applies postprocessing internally. Legacy YOLO11 models exported with CoreML NMS (`nms=True`) are also supported; the predictor detects the model's `nms` metadata flag at load time and dispatches to the correct path.
