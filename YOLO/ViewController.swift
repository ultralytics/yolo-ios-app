...
class ViewController: UIViewController {
  ...
  func show(
    predictions: [VNRecognizedObjectObservation], boxesAndValues: [(CGRect, Float, [Float])]
  ) {
    ...
    for i in 0..<boundingBoxViews.count {
      if i < (resultCount) && i < Int(slider.value) {
        ...
        case .human:
          ...
          innerTexts =
            "weight: " + String(format: "%.2f", humanFeatures.weight) + "\n" + "height: "
            + String(format: "%.2f", humanFeatures.height) + "\n" + "age: "
            + String(humanFeatures.age) + "\n" + humanFeatures.gender + ": "
            + String(format: "%.2f", humanFeatures.genderConfidence) + "\n" + humanFeatures.race
            + ": " + String(format: "%.2f", humanFeatures.raceConfidence)
          boxColor = .red
