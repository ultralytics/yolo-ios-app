...
class ViewController: UIViewController {
  ...
  override func viewDidLoad() {
    ...
    NotificationCenter.default.addObserver(
      self, selector: #selector(updateDeveloperMode), name: .settingsChanged, object: nil)
    slider.value = 30
        ...
        if developerMode {
          if save_detections {
            saveQueue.async { [self] in
              ...
              case .detect:
                str += String(
                  format: "%.3f,%.3f,%.3f,%@, %.2f,%.3f,%.3f,%.3f,%.3f\n",
              ...
              case .human:
                ...
                str += String(
                  format:
                    "%.3f,%.3f,%.3f,%@, %.2f,%.3f,%.3f,%.3f,%.3f,%.2f,%.2f,%d,%@,%.2f,%@,%.2f\n",
                  sec_day, freeSpace(), UIDevice.current.batteryLevel, id, confidence,
                  rect.origin.x, rect.origin.y, rect.size.width, rect.size.height, person.weight,
                  person.height, person.age, person.gender, person.genderConfidence, person.race,
                  person.raceConfidence)
              }
