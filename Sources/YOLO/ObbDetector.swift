...
class ObbDetector: BasePredictor, @unchecked Sendable {
    ...
    override func processObservations(for request: VNRequest, error: Error?) {
        if let results = request.results as? [VNCoreMLFeatureValueObservation] {
            ...
            if let prediction = results.first?.featureValue.multiArrayValue {
                ...
                self.currentOnResultsListener?.on(result: YOLOResult(orig_shape: inputSize, boxes: [],obb: obbResults, speed: 0, names: labels))
                self.updateTime()
            }
        }
    }
    ...
    func postProcessOBB(
        ...
        let keep   = nonMaxSuppressionOBB(boxes: boxes,
                                          scores: scores,
...
func polygonIntersection(subjectPolygon: Polygon, clipPolygon: Polygon) -> Polygon {
    ...
    for i in 0..<clipEdgeCount {
        ...
        for j in 0..<(closedInputPolygon.count - 1) {
            ...
            let currentInside = isInside(point: currentPoint,
                                         edgeStart: currentEdgeStart,
                                         edgeEnd: currentEdgeEnd)
            let nextInside    = isInside(point: nextPoint,
                                         edgeStart: currentEdgeStart,
                                         edgeEnd: currentEdgeEnd)
            ...
            if currentInside && nextInside {
                ...
                if let intersec = computeIntersection(p1: currentPoint,
                                                      p2: nextPoint,
...
func isInside(point: CGPoint,
              edgeStart: CGPoint,
              edgeEnd: CGPoint) -> Bool
{
    ...
    let cross = (edgeEnd.x - edgeStart.x) * (point.y - edgeStart.y)
...
func computeIntersection(p1: CGPoint,
                         p2: CGPoint,
                         clipStart: CGPoint,
                         clipEnd: CGPoint) -> CGPoint?
{
    ...
    let x3 = clipStart.x; let y3 = clipStart.y
    let x4 = clipEnd.x;   let y4 = clipEnd.y
...
struct OBBInfo {
    ...
    func iou(with other: OBBInfo) -> Float {
        let interPoly  = polygonIntersection(subjectPolygon: polygon,
